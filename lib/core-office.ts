...
export async function saveStudentFeeAddOnCore(
  user: AccessUser,
  input: { id?: string; studentId: string; label?: string; kind?: string; amount?: number; cadence?: string; startsPeriod?: string; endsPeriod?: string; active?: boolean }
) {
  ...
  const data = {
    ...
    cadence,
    startsPeriod: normalizeFeePeriod(input.startsPeriod) || "",
    endsPeriod: normalizeFeePeriod(input.endsPeriod) || "",
    active: input.active === false ? false : true,
  };
  const id = String(input.id || "");
  if (id) {
    const existing = await prisma.studentFeeAddOn.findUnique({ where: { id } });
    if (!existing || existing.studentId !== studentId) throw new Error("Add-on not found");
    await prisma.studentFeeAddOn.update({ where: { id }, data });
    return;
  }
  await prisma.studentFeeAddOn.create({ data });
}

export async function removeStudentFeeAddOnCore(user: AccessUser, input: { id: string }) {
  need(user, "people.edit");
  const id = String(input.id || "");
