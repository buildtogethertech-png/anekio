...
export function defaultLayout(type: string): DocumentLayout {
  ...
  if (type === "FEE_INVOICE" || type === "PAYMENT_RECEIPT") {
    ...
    const header = paid ? "#B45309" : "#0369A1";
    const ink = "#0F172A";
    const muted = "#64748B";
    const rule = "#E2E8F0";
    const wash = "#F8FAFC";
    const e = element;
    const label = (id: string, x: number, y: number, w: number, value: string) =>
      e(id, "TEXT", x, y, w, 1.2, { value, fontSize: 7, fontWeight: "bold", color: muted });
    const value = (id: string, x: number, y: number, w: number, field: string, name: string, extra: Partial<DocumentElement> = {}) =>
      e(id, "FIELD", x, y, w, 1.9, { field, label: name, fontSize: 11, fontWeight: "bold", color: ink, ...extra });
    return {
      elements: [
        e("page-bg", "SHAPE", 0, 0, 100, 100, { background: "#FFFFFF", locked: true }),
        e("header", "SHAPE", 0, 0, 100, 12.4, { background: header, locked: true }),
        e("school-logo", "IMAGE", 4.2, 2.2, 7.2, 8, { field: "school.logoPath", label: "Logo" }),
        e("school-name", "FIELD", 13.2, 2.4, 52, 3.4, { field: "school.name", label: "School name", fontSize: 18, fontWeight: "bold", color: "#FFFFFF", locked: true }),
        e("school-address", "FIELD", 13.2, 6, 52, 1.6, { field: "school.address", label: "School address", fontSize: 8, color: "#E2E8F0" }),
        e("school-contact", "FIELD", 13.2, 7.8, 52, 1.6, { field: "school.contact", label: "School contact", fontSize: 8, color: "#E2E8F0" }),
        e("doc-kind", "TEXT", 68, 2.6, 28, 1.4, { value: paid ? "OFFICIAL RECEIPT" : "FEE INVOICE", fontSize: 8, fontWeight: "bold", align: "right", color: "#E2E8F0" }),
        e("document-number", "FIELD", 68, 4.4, 28, 2.2, { field: "document.number", label: "Document number", fontSize: 12, fontWeight: "bold", align: "right", color: "#FFFFFF" }),
        e("academic-year", "FIELD", 68, 7, 28, 1.6, { field: "school.academicYear", label: "Academic year", fontSize: 8, align: "right", color: "#E2E8F0" }),
        e("header-rule", "SHAPE", 0, 12.4, 100, 0.35, { background: brand, locked: true }),
        e("title", "TEXT", 4.2, 14.2, 44, 2.4, { value: paid ? "Payment receipt" : "Fee invoice", fontSize: 16, fontWeight: "bold", color: ink }),
        label("status-l", 4.2, 16.8, 10, "STATUS"),
        value("status", 14.4, 16.6, 22, "fees.status", "Fee status", { color: brand, fontSize: 11 }),
        e("meta-rule", "LINE", 52, 14.4, 0.15, 5.2, { color: rule }),
        label("date-l", 54, 14.2, 18, "ISSUE DATE"),
        value("date", 54, 15.4, 18, "document.issueDate", "Issue date"),
        label("due-l", 74, 14.2, 22, paid ? "RECEIVED ON" : "DUE DATE"),
        value("due", 74, 15.4, 22, paid ? "fees.receivedAt" : "document.dueDate", paid ? "Received at" : "Due date"),
        e("bill-rule", "SHAPE", 4.2, 19.2, 91.6, 0.12, { background: rule, locked: true }),
        label("bill-l", 4.2, 20, 20, "BILL TO"),
        e("student-name", "FIELD", 4.2, 21.4, 42, 2.4, { field: "student.name", label: "Student name", fontSize: 15, fontWeight: "bold", color: ink }),
        label("class-l", 4.2, 24.2, 14, "CLASS"),
        value("class", 4.2, 25.4, 16, "student.classLabel", "Class"),
        label("adm-l", 22, 24.2, 18, "ADMISSION NO"),
        value("admission", 22, 25.4, 20, "student.admissionNo", "Admission number"),
        label("parent-l", 46, 24.2, 22, "PARENT / GUARDIAN"),
        value("parent", 46, 25.4, 24, "student.parent", "Guardian"),
        label("phone-l", 72, 24.2, 12, "PHONE"),
        value("phone", 72, 25.4, 23.8, "student.parentPhone", "Guardian phone"),
        label("term-l", 4.2, 28, 14, "FEE TERM"),
        value("term", 4.2, 29.2, 40, "fees.term", "Fee term"),
        e("table-head", "SHAPE", 4.2, 32.2, 91.6, 2.4, { background: header, locked: true }),
        e("table-title", "TEXT", 5.4, 32.5, 60, 1.8, { value: paid ? "PAYMENTS RECORDED" : "FEE PARTICULARS", fontSize: 8, fontWeight: "bold", color: "#FFFFFF" }),
        e("data-table", "TABLE", 4.2, 34.6, 91.6, 22.4, { field: "fees.lines", label: "Fee line items", fontSize: 10, background: header }),
        e("sum-panel", "SHAPE", 58, 58.2, 37.8, 12.6, { background: wash, borderColor: rule, locked: true }),
        label("sum-1-l", 60, 58.8, 16, "TOTAL BILLED"),
        value("amount", 76, 58.6, 18, "fees.amount", "Amount", { fontSize: 12, align: "right" }),
        label("sum-2-l", 60, 61.4, 16, "AMOUNT PAID"),
        value("paid", 76, 61.2, 18, "fees.paid", "Amount paid", { fontSize: 12, align: "right" }),
        e("sum-3", "SHAPE", 58, 64.2, 37.8, 6.6, { background: header, locked: true }),
        e("sum-3-l", "TEXT", 60, 64.8, 16, 1.4, { value: paid ? "RECEIPT TOTAL" : "AMOUNT DUE", fontSize: 7, fontWeight: "bold", color: "#E2E8F0" }),
        e("balance", "FIELD", 74, 65.8, 20, 3.6, { field: paid ? "fees.paid" : "fees.due", label: paid ? "Amount paid" : "Amount due", fontSize: 18, fontWeight: "bold", align: "right", color: "#FFFFFF" }),
        ...(paid
          ? [
              e("audit-card", "SHAPE", 4.2, 58.2, 51.6, 12.6, { background: "#FFFFFF", borderColor: rule, locked: true }),
              label("method-l", 5.6, 58.8, 22, "PAYMENT METHOD"),
              value("audit-method", 5.6, 60.2, 22, "fees.method", "Payment method"),
              label("ref-l", 29, 58.8, 24, "REFERENCE"),
              value("audit-ref", 29, 60.2, 24.4, "fees.reference", "Payment reference"),
              label("by-l", 5.6, 63, 22, "RECEIVED BY"),
              value("audit-by", 5.6, 64.4, 22, "fees.receivedBy", "Received by"),
              label("at-l", 29, 63, 24, "RECEIVED AT"),
              value("audit-at", 29, 64.4, 24.4, "fees.receivedAt", "Received at"),
              e("audit-note", "FIELD", 5.6, 66.6, 48, 1.6, { field: "fees.receivedNote", label: "Payment note", fontSize: 8, color: "#475569" }),
              e("legal", "TEXT", 5.6, 68.2, 48, 1.8, { value: "Official acknowledgement of payment recorded in the school fee ledger.", fontSize: 7, color: muted }),
            ]
          : [
              e("pay-card", "SHAPE", 4.2, 58.2, 51.6, 12.6, { background: "#FFFFFF", borderColor: rule, locked: true }),
              label("upi-l", 5.6, 58.8, 22, "UPI ID"),
              value("upi", 5.6, 60.2, 22, "fees.upiId", "UPI ID"),
              label("bank-l", 29, 58.8, 24, "BANK"),
              value("bank", 29, 60.2, 24.4, "fees.bankName", "Bank name"),
              label("acct-l", 5.6, 63, 48, "ACCOUNT"),
              value("account", 5.6, 64.4, 48, "fees.account", "Bank account", { fontSize: 9 }),
              e("pay-note", "TEXT", 5.6, 67.2, 48, 2.8, { value: "Pay the amount due using the school UPI or bank details. Line items follow the Fees register.", fontSize: 7, color: muted }),
            ]),
        e("sign", "SIGNATURE", 4.2, 74.2, 22, 8, { field: "school.signPath", label: "Authorised signature" }),
        e("sign-l", "TEXT", 4.2, 82.4, 22, 1.4, { value: "Authorised signatory", fontSize: 7, align: "center", color: muted }),
        e("stamp", "STAMP", 30, 74.2, 12, 8.4, { field: "school.stampPath", label: "School stamp" }),
        e("verify", "VERIFY_QR", 84.2, 73.8, 10.4, 10.4, { label: "Verification QR", locked: true }),
        e("qr-l", "TEXT", 83.2, 84.4, 12.4, 1.4, { value: "VERIFY", fontSize: 7, fontWeight: "bold", align: "center", color: muted }),
        e("footer-line", "LINE", 4.2, 87.2, 91.6, 0.15, { color: rule }),
        e("footer", "FIELD", 4.2, 88.2, 52, 1.6, { field: "school.contact", label: "School contact", fontSize: 8, color: muted }),
        e("footer-web", "FIELD", 58, 88.2, 37.8, 1.6, { field: "school.website", label: "School website", fontSize: 8, align: "right", color: muted }),
      ],
    };
  }
