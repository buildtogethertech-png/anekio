  ...
  return (
    <View className="min-h-0 flex-1">
      ...
      <View className="mb-3 flex-row flex-wrap gap-2">
        <Chip label="Fee Register" active={tab === "register"} onPress={() => setTab("register")} />
        <Chip label="Configure fees" active={tab === "templates"} onPress={() => setTab("templates")} />
      </View>
      {tab === "register" ? <FeeRegister /> : null}
      {tab === "templates" ? (
        <Card className="mb-6 p-4">
          ...
          <View className="mb-3 flex-row flex-wrap gap-2">
            {(data?.classes ?? []).map((c) => (
              <Chip key={c.id} label={c.label} active={classId === c.id} onPress={() => setClassId(c.id)} />
            ))}
          </View>
          {classId === "all" ? (
