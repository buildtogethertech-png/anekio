...
function admissionFieldHtml(field: AdmissionFormField) {
  ...
  if (field.type === "textarea") {
    return `<div class="field full">${label}<textarea name="${esc(field.id)}"${required}></textarea></div>`;
  }
  if (field.type === "select") {
    return `<div class="field">${label}<select name="${esc(field.id)}"${required}><option value="">Select</option>${field.options.map((option) => `<option value="${esc(option)}">${esc(option)}</option>`).join("")}</select></div>`;
  }
  const type = field.type === "phone" ? "tel" : field.type;
  return `<div class="field">${label}<input name="${esc(field.id)}" type="${type}"${required}></div>`;
}

export async function schoolWebsiteHtml(slug?: string) {
  const school = await prisma.schoolConfig.findUnique({ where: { id: "school" } });
  if (!school?.websiteEnabled) return null;
  if (slug && slug !== school.websiteSlug) return null;
...
<body>
  ...
  <main>
    ...
    <section id="platform" class="section platform"><div class="wrap platform-grid">
      <div><span class="eyebrow" style="color:${theme.accent}">Powered by Cultivate</span><h2 style="color:white">Your child follows a world-class digital school system.</h2><p style="color:#cbd5e1;line-height:1.8">This school uses Cultivate to keep attendance, marks, fees, notices, documents, admissions and parent communication organised in one place. Parents do not have to chase scattered messages — important updates stay structured and easy to follow.</p></div>
  ...
  <footer class="footer"><div class="wrap"><div><strong>${esc(school.name)}</strong><div class="small">${esc(place)}</div></div><div class="small">Admissions website powered by Cultivate</div></div></footer>
</body>
</html>`;
}

export async function createAdmissionLeadFromWebsite(slug: string, input: Record<string, unknown>) {
  const school = await prisma.schoolConfig.findUnique({ where: { id: "school" } });
  if (!school?.websiteEnabled) throw new Error("Admissions website is not active.");
  if (slug !== school.websiteSlug) throw new Error("Admissions website not found.");
